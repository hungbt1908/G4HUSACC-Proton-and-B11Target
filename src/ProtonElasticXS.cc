...
//
//
// G4 Physics class: ProtonElasticXS for pA elastic cross sections
...
//
G4_DECLARE_XS_FACTORY(ProtonElasticXS);
...
ProtonElasticXS::ProtonElasticXS():G4VCrossSectionDataSet(Default_Name()), nPoints(128), nLast(nPoints-1)
{
...
ProtonElasticXS::~ProtonElasticXS()
{
...
void
ProtonElasticXS::CrossSectionDescription(std::ostream& outFile) const
{
    outFile << "ProtonElasticXS provides the elastic cross\n"
            << "section for proton nucleus scattering as a function of incident\n"
            << "momentum. The cross section is calculated using M. Kossov's\n"
            << "CHIPS parameterization of cross section data.\n";
}

G4bool ProtonElasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int, G4int,    
				 const G4Element*,
				 const G4Material*)
{
  return true;
...
G4double ProtonElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ, G4int A,  
							const G4Isotope*,
							const G4Element*,
							const G4Material*)
{
...
G4double ProtonElasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN, G4int)
{
  ...
  {                                  // The nucleus with projPDG is found in AMDB
    if(colN[i]==tgN && colZ[i]==tgZ) // Isotope is foind in AMDB
    {
      ...
      if(lastP == pMom)              // Do not recalculate
      {
        ...
        return 1000*lastCS*millibarn;     // Use theLastCS
      }
...
G4double ProtonElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I,
...
G4double ProtonElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG,
                                                  G4int tgZ, G4int tgN)
{
  ...
  if(PDG==2212)
  {
    ...
    if(LP>ILP)
    {
      ...
      if(ini<nPoints)
      {
        ...
        else G4cout<<"*Warning*ProtonElasticXS::GetPTables: PDG="<<PDG<<", Z="
  ...
  {
    // G4cout<<"*Error*ProtonElasticXS::GetPTables: PDG="<<PDG<<", Z="<<tgZ
    //       <<", N="<<tgN<<", while it is defined only for PDG=2212"<<G4endl;
    // throw G4QException("ProtonElasticXS::GetPTables: only pA're implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for PDG=2212 (p)" << G4endl;
    G4Exception("ProtonElasticXS::GetPTables()", "HAD_CHPS_0000",
                FatalException, ed);
  }
  return ILP;
}
...
G4double ProtonElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if(PDG!=2212) G4cout<<"**Warning*ProtonElasticXS::GetExT:PDG="<<PDG<<G4endl;
  if(onlyCS) G4cout<<"**Warning*ProtonElasticXS::GetExchanT:onlyCS=1"<<G4endl;
...
G4double ProtonElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  static const G4double GeVSQ=gigaelectronvolt*gigaelectronvolt;
  if(onlyCS) G4cout<<"*Warning*ProtonElasticXS::GetSlope:onlyCS=true"<<G4endl;
  if(lastLP<-4.3) return 0.;          // S-wave for p<14 MeV/c (kinE<.1MeV)
  if(PDG!=2212)
  {
    // G4cout<<"*Error*ProtonElasticXS::GetSlope: PDG="<<PDG<<", Z="<<tgZ<<", N="
    //       <<tgN<<", while it is defined only for PDG=2212"<<G4endl;
    // throw G4QException("ProtonElasticXS::GetSlope: pA are implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for PDG=2212 (p)" << G4endl;
    G4Exception("ProtonElasticXS::GetSlope()", "HAD_CHPS_0000",
                FatalException, ed);
  }
  if(theB1<0.) theB1=0.;
...
G4double ProtonElasticXS::GetHMaxT()
{
...
G4double ProtonElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ,
                                                    G4int tgN)
{
  if(PDG!=2212) G4cout<<"*Warning*ProtonElasticXS::GetTabV:PDG="<<PDG<<G4endl;
...
G4double ProtonElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN,
                                                G4double pP)
{
  ...
  {
    // G4cout<<"*Error*ProtonElasticXS::GetQ2max: PDG="<<PDG<<", Z="<<tgZ<<", N="
    //       <<tgN<<", while it is defined only for p projectiles & Z_target>0"<<G4endl;
    // throw G4QException("ProtonElasticXS::GetQ2max: only pA are implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for p projectiles & Z_target>0" << G4endl;
    G4Exception("ProtonElasticXS::GetQ2max()", "HAD_CHPS_0000",
                FatalException, ed);
    return 0;
  }
}